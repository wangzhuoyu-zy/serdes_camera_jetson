# FG-4CH FZCAM Jetson AGX Thor JetPack 7.2 (R39.2) SOP

目录：JetPack_7.2_R39.2_4CH_YUV_Cameras

## 1. 目标

- 在 Jetson **AGX Thor (P3971-0000 + P3834-xxxx 载板)** + FG24/12-4CH（YUV）SerDes 套件上完成驱动 / DTBO / 应用升级。
...
- 驱动与 dtbo
  ...
  - `rootfs/boot/tegra264-p3971-camera-fg-4ch-overlay.dtbo`
    - 内含 `overlay-name = "Jetson Camera Thor_FG_4CH"`、`jetson-header-name = "Jetson AGX CSI Connector"`
...
#I2C总线 /dev/i2c-9 对应 /dev/video0~3
#I2C总线 /dev/i2c-12 对应 /dev/video4~7
```

FG-4CH 套件预期输出 4 个视频节点（具体编号以 Thor 平台枚举为准）。
...
- 处理：
  ...
  ```
  必须输出 `Jetson Camera Thor_FG_4CH`。
