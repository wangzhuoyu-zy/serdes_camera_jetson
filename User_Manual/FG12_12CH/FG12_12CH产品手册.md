...
● FG12_12CH GMSL采集板是竹影智能/方竹科技基于ADI Deserializer芯片MAX96712开发的8路GMSL采集板，以下简称采 集板。
● 此采集板可同时接入至多8个GMSL相机并输出CSI信号到NVIDIA® Jetson AGX Orin™ 开发套件(亦可接于AGX Xavier开发套件上)
●通过软件配置让MAX96712自适应GMSL1与GMSL2 3G/6G不同速率的GMSL相机。
●NVIDIA Jetson® AGX Orin™ Devkit开发套件上120Pin连接器不能提供相机所需电源，因此采集板通过一个可插拔的12V-24V的DC电源外部连接器（可与套件采用同一电源适配器）。
FG12_12CH GMSL相机平台是一个扩展板，允许多达8个相机连接到Jetson® AGX Orin™/Xavier™模块，它与NVIDIA Jetson® AGX Orin™/Xavier™开发套件完全兼容。自适应的工作在不同频率即通过软件配置同时兼容GMSL1与GMSL2协议接口从而可以采用市面上许多不同类型的GMSL相机。GMSL相机的电源由PoC(Power over Coax)提供，因此所有数据、控制信号和电源都通过一根50欧姆同轴电缆发送，这使得相机的电缆布线灵活且具备较强的电磁抗干扰能力，易于在自主移动机器人（AMR）机器人技术、无人机（UAV）及汽车等应用中安装及使用。

### 1.1 产品图片

![产品图片1](../images/FG12_12CH/pic1.png)

![产品图片2](../images/FG12_12CH/pic2.png)
...
| 解串器 | ADI MAX96712，每个解串器接入一个4-Lane MIPI |
...
- FG12_12CH载板支持输入电源范围：12V~24V，电流：2A以上
...
![产品框图](../images/FG12_12CH/block_diagram.png)
...
![位号图](../images/FG12_12CH/component_placement.png)

![位号布局图2](../images/FG12_12CH/component_placement_2.png)
...
| 序号 | 名称 | 标识 | 备注 |
|------|------|------|------|
| 1 | CAM0 | J8 | FAKRA同轴连接器，GMSL输入接口 |
| 2 | CAM1 | J9 | FAKRA同轴连接器，GMSL输入接口 |
| 3 | CAM2 | J10 | FAKRA同轴连接器，GMSL输入接口 |
| 4 | CAM3 | J11 | FAKRA同轴连接器，GMSL输入接口 |
| 5 | CAM4 | J2 | FAKRA同轴连接器，GMSL输入接口 |
| 6 | CAM5 | J3 | FAKRA同轴连接器，GMSL输入接口 |
| 7 | CAM6 | J4 | FAKRA同轴连接器，GMSL输入接口 |
| 8 | CAM7 | J5 | FAKRA同轴连接器，GMSL输入接口 |
| 9 | 电源输入接口 | J1 | 规格：HC-5557-2*2A |
| 10 | 外部输入触发信号接口 | J15 | 2*3P直插 |
| 11 | 金手指连接器-模块连接接口 | J14 | BTB高速连接器 |
...
### 7.2 J2/J3/J4/J5/J8/J9/J10/J11 - FAKRA连接器

**功能：** GMSL相机应用接口

**标识：** J2、J3、J4、J5、J8、J9、J10、J11

**类型/型号：** 安费诺 2FA1-NZSP-PCBB6

**功能简介：** GMSL相机应用接口基于V4L2，为驱动和应用程序提供了一套统一的接口规范，方便进行应用开发。
...
支持动态接入和加载相机，通过运行`sudo fzcam_ui`运行并配置CAM0~7（亦兼容12路配置），进而选择不同相机品牌和Sensor型号。
...
![相机配置界面](../images/FG12_12CH/camera_config.png)
