...
●FG24_4CH GMSL板是竹影智能/方竹科技基于 ADI Deserilaizer芯片MAX96724开发的4路GMSL采集板，以下简称采集板 。
●此采集板可同时接入至多4个GMSL相机并输出CSI信号到NVIDIA® Jetson Orin™ NX开发套件(亦可接于Xavier NX开发套件上)
●此采集板可通过软件配置的方式让MAX96724自适应的解析GMSL1与GMSL2不同速率的GMSL相机。
●NVIDIA Jetson® Orin/X avier™NX开发套件上22Pin连接器不能提供相机所需电源，因此采集板通过一个可插拔的12V-24V的DC电源外部连接器（可与套件采用同一电源适配器）
FG24_4CH GMSL相机平台是一个扩展板，允许多达4个相机连接到Jetson® Orin™/Xavier™ NX模块，它与NVIDIA Jetson® Orin™/Xavier™ NX Devkit开发套件完全兼容。自适应的工作在不同频率即通过软件配置同时兼容GMSL1与GMSL2协议接口。

易于在自主移动机器人（AMR）机器人技术、无人机（UAV）及汽车等应用中安装及使用。

### 1.1 产品图片

![产品图片1](../images/FG24_4CH/pic1.png)

![产品图片2](../images/FG24_4CH/pic2.png)

![产品图片3](../images/FG24_4CH/pic3.png)
...
| 保护功能 | 电路内置过压及短路保护 |
| 电源控制 | 软件可控制相机的供电 |
| 尺寸适配 | 尺寸小巧适配Jetson Orin/Xavier NX Devkit |
...
| 解串器 | ADI MAX96724 |
| MIPI输出 | 支持2/4-Lane CSI-2 v1.3 |
| 相机连接器 | 安费诺单端卧式同轴FAKRA连接器 |
| PoC供电 | 4路相机供电由GMSL板提供，PoC 12V (DC Input > 12V) |
| 电源输入 | 支持外接12V~24V @ 3A MAX |
| 工作温度 | -20℃ ~ +65℃（-4℉ ~ +149℉） |
| 质保支持 | 一年质保和技术支持 |
...
- FG24_4CH载板支持输入电源范围：12V~24V，电流：2A以上
...
![产品框图](../images/FG24_4CH/block_diagram.png)
...
![位号图](../images/FG24_4CH/component_placement.png)
...
| 2 | CAMA | J2 | FAKRA同轴连接器，GMSL输入接口 |
| 3 | CAMB | J3 | FAKRA同轴连接器，GMSL输入接口 |
| 4 | CAMC | J4 | FAKRA同轴连接器，GMSL输入接口 |
| 5 | CAMD | J5 | FAKRA同轴连接器，GMSL输入接口 |
...
如相机需同步触发，请连接AGX套件PIN33->FG24-4CH的MAX1_CSI0_SYNC, AGX套件PIN34->FG24-4CH的GND。

### 8.2 相机配置界面

![相机配置界面](../images/FG24_4CH/camera_config.png)
