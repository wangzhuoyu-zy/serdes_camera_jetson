...
●  FG 96_2CH GMSL板是竹影智能/方竹科技基于ADI Deserializer芯片MAX9296A开发的2路GMSL采集板，以下简称采集板 。
●  此 采集板可同时接入至多2个GMSL相机并输出CSI信号到NVIDIA® Jetson AGX Orin™NX开发套件(亦可接于Xavier NX开发套件上)
●此采集板可通过软件配置的方式让MAX9296A自适应的解析GMSL1与GMSL2不同速率的GMSL相机。
●NVIDIA Jetson® AGX Orin™NX开发套件上22Pin连接器不能提供相机所需电源，因此采集板通过一个可插拔的12V-24V的DC电源外部连接器（可与套件采用同一电源适配器）

FG96-2CH GMSL相机平台是一个扩展板，允许多达2个相机连接到Jetson® AGX Orin™/Xavier™模块，它与NVIDIA Jetson® AGX Orin™/Xavier™开发套件完全兼容。自适应的工作在不同频率即通过软件配置同时兼容GMSL1与GMSL2协议接口。

易于在自主移动机器人（AMR）机器人技术、无人机（UAV）及汽车等应用中安装及使用。

### 1.1 产品图片

![产品图片1](../images/FG96_2CH/pic1.png)

![产品图片2](../images/FG96_2CH/pic2.png)

![产品图片3](../images/FG96_2CH/pic3.png)
...
| 相机输入数量 | 2路GMSL2或GMSL1相机（RGB/RGBD等均兼容支持） |
| 解串器 | ADI MAX96718 |
| MIPI输出 | 支持4-Lane CSI |
| 相机连接器 | 单端立式FAKRA连接器 |
| PoC供电 | 2路相机供电由GMSL板提供，POC 12V电压 |
...
- FG96_2CH载板支持输入电源范围：12V~24V，电流：2A以上
...
![产品框图](../images/FG96_2CH/block_diagram.png)

**注意：**

1. I2C总线号为硬件位置(匹配连接器J410、J411引脚)。总线号不一定与软件中列出的相对应。
...
![位号图](../images/FG96_2CH/component_placement.png)
...
| 序号 | 名称 | 标识 | 备注 |
|------|------|------|------|
| 1 | 电源输入接口 | J101 | 支持12~24V DC输入 |
| 2 | CSI0 | J411 | Molex 22Pin ZIF连接器(4Lane CSI)接口 |
| 3 | CSI1 | J410 | Molex 22Pin ZIF连接器(4Lane CSI)接口 |
| 4 | CAM0 | J708 | FAKRA同轴连接器，GMSL输入接口 |
| 5 | CAM1 | J709 | FAKRA同轴连接器，GMSL输入接口 |
| 6 | 外部输入触发信号连接器 | J102 | 1*3P |
...
### 7.2 J410/J411 - Molex 22Pin CSI输出
...
| 9 | CKB P | 21 | CAM_I2C_SDA |
| 10 | GND | 22 | VCC_3V3 |
| 11 | DB2N | 23 | GND |
| 12 | DB2P | 24 | GND |

### 7.2 J708/J709 - FAKRA连接器

**功能：** GMSL相机应用接口

**标识：** J708、J709

**类型/型号：** 安费诺单端立式FAKRA连接器
...
支持动态接入和加载相机，通过运行`sudo fzcam_ui`运行并配置CAM0~1，进而选择不同相机品牌和Sensor型号。
...
![相机配置界面](../images/FG96_2CH/camera_config_cn.png)
