...
● FG 12-16CH GMSL板是竹影智能/方竹科技基于ADI Deserializer芯片MAX96712开发的16路GMSL采集板，以下简称采集 板。
●  此 采集板可同时接入至多16个GMSL相机并输出CSI信号到NVIDIA® Jetson AGX Orin™/Xavier™开发套件。
●  此 采集板可通过软件配置的方式让MAX96712自适应的解析GMSL1与GMSL2 3G/6G不同速率的GMSL相机。
●NVIDIA Jetson® AGX Orin™/Xavier™开发套件上120Pin连接器不能提供相机所需电源，因此采集板通过一个可插拔的12V~24V的
DC电源外部连接器（可与套件采用同一电源适配器）。

FG12_16CH GMSL相机平台是一个扩展板，允许多达16个相机连接到Jetson® AGX Orin™/Xavier™模块，它与NVIDIA Jetson® AGX Orin™/Xavier™开发套件完全兼容。自适应的工作在不同频率即通过软件配置同时兼容GMSL1与GMSL2协议接口从而可以采用市面上许多不同类型的GMSL相机。

易于在自主移动机器人（AMR）机器人技术、无人机（UAV）及汽车等应用中安装及使用。

### 1.1 产品图片

![产品图片1](../images/FG12_16CH/pic1.png)

![产品图片2](../images/FG12_16CH/pic2.png)
...
| 相机输入数量 | 16路GMSL2或GMSL1相机（RGB/RGBD等均兼容支持） |
| 解串器 | ADI MAX96712 |
| MIPI输出 | 每个解串器接入一个4-Lane MIPI CSI-2 v1.3（总计16-Lanes） |
| 相机连接器 | 四合一TE规格FAKRA连接器 |
| PoC供电 | 16路相机供电由GMSL板提供，PoC 12V (DC Input > 12V) |
...
- FG12_16CH载板支持输入电源范围：12V~24V，电流：2A以上
...
![产品框图](../images/FG12_16CH/block_diagram.png)
...
![位号图](../images/FG12_16CH/component_placement.png)

![位号布局图2](../images/FG12_16CH/component_placement_2.png)
...
| 序号 | 名称 | 标识 | 备注 |
|------|------|------|------|
| 1 | CAM0~CAM3 | J14 | 四合一FAKRA同轴连接器，GMSL输入接口 |
| 2 | CAM4~CAM7 | J15 | 四合一FAKRA同轴连接器，GMSL输入接口 |
| 3 | CAM8~CAM11 | J16 | 四合一FAKRA同轴连接器，GMSL输入接口 |
| 4 | CAM12~CAM15 | J17 | 四合一FAKRA同轴连接器，GMSL输入接口 |
| 5 | 电源输入接口 | J1 | 规格：HC-5557-2*2A |
| 6 | 外部输入触发信号接口 | J19 | 1*6P直插 |
| 7 | 金手指连接器-模块连接接口 | J2 | BTB高速连接器 |
...
### 7.2 J14/J15/J16/J17 - 四合一FAKRA连接器

**功能：** GMSL相机应用接口

**标识：** J14、J15、J16、J17

**类型/型号：** TE规格四合一公头卧式FAKRA连接器
...
**类型/型号：** 120PIN 高度25mm QTH-060-07-F-D-A
...
支持动态接入和加载相机，通过运行`sudo fzcam_ui`运行并配置CAM0~15，进而选择不同相机品牌和Sensor型号。
...
![相机配置界面](../images/FG12_16CH/camera_config.png)
