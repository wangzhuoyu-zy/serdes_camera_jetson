...
red_print "------------FG24-8CH AGX Orin Devkit 固件升级Firmware Upgrade----------------------"
echo ''

str=$(cat /etc/nv_tegra_release)
if [[ $str == *$str_ver0* ]]; then
	echo $str_ver
	green_print 'JetPack R36.4.0 is corret, JetPack版本匹配自动升级'
else
	if [[ $str == *$str_ver3* ]]; then
		...
		if [[ $str == *$str_ver4* ]]; then
			...
			if [[ $str == *$str_ver7* ]]; then
				echo $str_ver
				green_print 'JetPack R36.5.7 is corret, JetPack版本匹配自动升级'
			else
				red_print 'JetPack version is not supported'
				exit 0
			fi
		fi
	fi
