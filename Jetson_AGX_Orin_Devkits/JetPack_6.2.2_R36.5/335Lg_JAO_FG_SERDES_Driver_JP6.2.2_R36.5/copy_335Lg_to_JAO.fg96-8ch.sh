...
# 8x335Lg
sudo cp tegra234-p3737-camera-g300-fg96-overlay.dtbo /boot/

# 4x335Lg+8xRGB
#sudo cp tegra234-p3737-camera-2x335Lg-rgbx6-fg96-8ch-overlay.dtbo /boot/
#sudo cp tegra234-p3737-camera-4x335Lg-rgbx4-fg96-8ch-overlay.dtbo /boot/

# 让用户选择相机配置模式
echo "请选择相机配置模式："
echo "0) CAM0~7 仅连接 335Lg 相机"
echo "1) CAM0~1 连接 335Lg 相机，CAM2~7 连接 RGB"
echo "2) CAM0~3 连接 335Lg 相机，CAM4~7 连接 RGB"
...
case $choice in
    0)
        # 仅 335Lg
        need_rgb=0
        sudo /opt/nvidia/jetson-io/config-by-hardware.py -n 2="Jetson Camera FG96_8CH_DPHY_8xG335Lg"
        ;;
    1)
        # 2x335Lg (CAM0~1) + 6xRGB_YUV
        need_rgb=1
        sudo /opt/nvidia/jetson-io/config-by-hardware.py -n 2="Jetson Camera FG96_8CH_2x335Lg_6xRGB"
        ;;
    2)
        # 4x335Lg (CAM0~3) + 4xRGB_YUV
        need_rgb=1
        sudo /opt/nvidia/jetson-io/config-by-hardware.py -n 2="Jetson Camera FG96_8CH_4x335Lg_4xRGB"
        ;;
    *)
        echo "无效输入，退出脚本"
        exit 1
