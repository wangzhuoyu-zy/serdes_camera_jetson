# FG24 8CH YUV Cameras JetPack 6.2.2 (R36.5.x) SOP

目录：JetPack_6.2.2_R36.5_JAO_FG24_8CH_YUV_Cameras

## 1. 目标

- 在 Jetson AGX Orin + FG24 8CH YUV Cameras 套件上完成固件/应用升级。
- 支持 8 通道 YUV 相机的配置和使用。

## 2. 升级包内容说明

- 升级脚本
  - fg24.8ch.JAO.R36.5.x.sh
- DTBO 设备树覆盖
...
脚本做的事情（关键点）：
- 检查 JetPack 版本是否为 R36.5.0/5.3/5.4/5.7
...
脚本会检查 `/etc/nv_tegra_release` 是否为 R36.5.0/5.3/5.4/5.7，若不匹配会退出。
