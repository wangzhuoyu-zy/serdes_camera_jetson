...
目录：`JetPack_7.2_R39.2_JAO_FG24_8CH_YUV_Cameras`

## 1. 目标

- 在 Jetson AGX Orin Devkit + FG24 8CH YUV Cameras 套件上完成固件/应用升级。
- 支持 JetPack 7.2 / R39.2.x 内核（Linux 6.8.12-tegra）。
- 支持 8 通道 YUV 相机的配置与出图。
...
- 演示截图（参考）
  - `pics_videos/fg96-8ch-fzcam-ui.png`（界面与 FG96 套件一致，仅作参考）
...
- 安装 DTBO 并通过 Jetson-IO 配置硬件：`Jetson Camera FG24CH_8xYUV_4Lane`
- 二次确认后重启
...
1. 确认相机连接正确（8 路 FAKRA 已插好，电源 12V 已上）
2. 重新执行升级脚本并重启
3. 检查 DTBO 是否被正确启用：
   ```bash
...
### 5.4 FG24 与 FG96 套件混淆

两个套件共用 `fzcam.ko` 驱动，但 DTBO 不同：

- FG24：`tegra234-p3737-camera-fzcam-fg24-8ch-4lanes.dtbo`，Jetson-IO 配置名 `Jetson Camera FG24CH_8xYUV_4Lane`
- FG96：`tegra234-p3737-camera-fzcam-fg96-8ch-4lanes.dtbo`，Jetson-IO 配置名 `Jetson Camera FG96_8CH_8xYUV`

混用 DTBO 会导致链路不通或画面异常。如已错装，请重新执行本目录下的升级脚本以恢复正确配置。